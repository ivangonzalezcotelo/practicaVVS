...
public class Insert {
    ...
    void onValidateFromInsertForm() {
		...
		if(position.getIndex() != launchPrice.length()){
			insertForm.recordError(launchPriceTextField,
					messages.format("error-incorrectNumberFormat", launchPrice));
		} else {
        ...
        try {
            productService.insertProduct(productName, description, launchPriceAsBigDecimal, sendInfo, timeRemaining , userSession.getUserProfileId() , category.getCatId());
